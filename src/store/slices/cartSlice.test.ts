...
describe('cartSlice', () => {
  ...
  it('deve adicionar um novo item ao carrinho', () => {
    const novoEstado = cartReducer(
      estadoAnterior,
      addItem({ id: '1', name: 'Item 1', price: 100, image: '' }),
    );
  ...
  it('deve incrementar à quantidade se o item já existe no carrinho', () => {
    let novoEstado = cartReducer(
      estadoAnterior,
      addItem({ id: '1', name: 'Item 1', price: 100, image: '' }),
    );
    novoEstado = cartReducer(
      novoEstado,
      addItem({ id: '1', name: 'Item 1', price: 100, image: '' }),
    );
    expect(novoEstado.items.length).toBe(1);
  ...
  it('deve incrementar à quantidade se o item já existe no carrinho', () => {
    ...
    novoEstado = cartReducer(
      novoEstado,
      addItem({ id: '2', name: 'Item 2', price: 100, image: '' }),
    );
  ...
  it('não deve fazer nada se item não existir', () => {
    ...
    const actual = cartReducer(
      previousState,
      updateQuantity({ id: '2', quantity: 5 }),
    );
    expect(actual.items).toHaveLength(1);
    expect(actual.items[0]).toEqual(mockItemCarrinho);
  });
